 ...
 * @param {Object} _dir 
 */
const addgambar = (userId, expired, _dir) => {
    const obj = { id: userId, expired: Date.now() + toMs(expired) }
    _dir.push(obj)
    fs.writeFileSync('./database/user/tebakgambar.json', JSON.stringify(_dir))
}
...
const isTebakGambar = (userId, _dir) => {
    let status = false
    Object.keys(_dir).forEach((i) => {
        if (_dir[i].id === userId) {
            status = true
...
const cekWaktuTembak = (_dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            console.log(`Waktu habis: ${_dir[position].id}`)
